...
export function setupPedidos(config) {
    ...
    window.excluirPedidoAtual = excluirPedidoAtual; // Exposta para redundância
...
function initListenersPedidos() {
    ...
    if(btnExcluir) btnExcluir.addEventListener('click', excluirPedidoAtual);
    ...
    let filtrados = pedidos.filter(ped => {
        ...
        if (filtroMesPed !== "" || filtroAnoPed !== "") {
            if (!ped.dataPedido) {
                ...
                const pAno = partes[0];
                const pMes = String(parseInt(partes[1]) - 1); 
...
// ==========================================================================
// 4. EDIÇÃO E EXCLUSÃO DE PEDIDOS
// ==========================================================================
...
async function atualizarPedido() {
    ...
    try {
        ...
        pedidos.sort((a, b) => {
            const dataA = a.dataPedido || "";
            const dataB = b.dataPedido || "";
...
/**
 * Realiza a exclusão segura do pedido do Firebase e da lista local.
 */
async function excluirPedidoAtual() {
    if (!pedidoEditando) {
        alert("Erro: Nenhum pedido selecionado para exclusão.");
        return;
    }

    const confirmacao = confirm(
        "⚠️ ATENÇÃO: AÇÃO IRREVERSÍVEL ⚠️\n\n" +
        "Você está prestes a EXCLUIR este pedido permanentemente.\n" +
        "1. Ele sumirá do Relatório Financeiro.\n" +
        "2. Ele sumirá da lista de Pedidos Realizados.\n\n" +
    ...
    try {
        ...
        alert("Erro ao tentar excluir. Veja o console.");
    } finally {
        const btnExcluir = document.getElementById('btnExcluirPedido');
        if(btnExcluir) {
