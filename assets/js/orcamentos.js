...
import { buscarDadosPrecificacao } from './precificacao.js'; // Novo: Conexão com Histórico
...
export async function initOrcamentos() {
    console.log("Inicializando Módulo Orçamentos v1.3.0 + Automação Financeira...");
    
    // Exposição de funções para o escopo global (Eventos Inline)
...
// ==========================================================================
// 4. LÓGICA DE NEGÓCIO: ORÇAMENTOS (REFATORADA)
// ==========================================================================
...
function adicionarProduto() {
    ...
    // Armazena dados financeiros ocultos na linha (dataset)
    ...
    newRow.innerHTML = `
        ...
        <td style="position: relative;">
            <div class="search-wrapper-integrated">
                <input type="text" class="search-input-integrated produto-descricao" placeholder="Buscar produto..." oninput="tratarBuscaProdutoOrcamento(this)">
                <span class="search-icon-integrated">🔍</span>
        ...
        <td style="width: 50px;"><button type="button" onclick="excluirProduto(this)" style="background:#e53935; padding: 5px 10px; color: white; border: none; border-radius: 4px; cursor: pointer;">X</button></td>
    `;
}
...
function tratarBuscaProdutoOrcamento(input) {
    ...
    btnClear.style.display = termo ? 'block' : 'none';
...
function selecionarProdutoOrcamento(input, dados) {
    ...
    // Dataset para cálculo financeiro (Soma custos diretos e indiretos)
...
function atualizarTotais() {
    ...
    document.querySelectorAll("#tabelaProdutos tbody tr").forEach(row => {
        ...
        // Cálculos Financeiros Ocultos (acumulado por quantidade)
    ...
    // Atualização do Painel de Feedback Financeiro
...
async function gerarOrcamento() {
    ...
    document.querySelectorAll("#tabelaProdutos tbody tr").forEach(row => {
        dados.produtos.push({
            ...
            // Metadados financeiros salvos para futuras referências no pedido
...
function editarOrcamento(id) {
    ...
    if (orc.produtos && orc.produtos.length > 0) {
        orc.produtos.forEach(p => {
            ...
            // Re-hidratando datasets se existirem
            row.dataset.custoMat = p.custoBase || 0;
            row.dataset.maoObra = p.maoObraBase || 0;
            const lucroBase = (p.valorUnit) - ( (p.custoBase || 0) + (p.maoObraBase || 0) );
            row.dataset.lucro = lucroBase;

            row.innerHTML = `
                <td style="width: 80px;"><input type="number" class="produto-quantidade" value="${p.quantidade}" min="1" onchange="atualizarTotais()"></td>
                <td style="position: relative;">
                    <div class="search-wrapper-integrated">
                        <input type="text" class="search-input-integrated produto-descricao" value="${p.descricao}" oninput="tratarBuscaProdutoOrcamento(this)">
                        <span class="search-icon-integrated">🔍</span>
...
// ==========================================================================
// 5. ORDENAÇÃO E RENDERIZAÇÃO DA TABELA DE LISTAGEM
// ==========================================================================
...
async function gerarPedido(orcamentoId) {
    ...
    // Agora priorizamos os dados que já estão no Orçamento (Dataset)
    ...
    if (custoMaoDeObra === 0 || margemLucro === 0 || custosTotais === 0) {
        if (utils && utils.showToast) {
            utils.showToast("Alerta Pedagógico: O pedido será gerado com pendências financeiras.", "warning");
        }
    }

    if (resultadoFinanceiro.status === 'prejuizo') {
        let mensagemPrejuizo = `⛔ PREJUÍZO OPERACIONAL DETECTADO!\n`;
        mensagemPrejuizo += `O valor cobrado não cobre nem os materiais.\n`;
        mensagemPrejuizo += `Receita: ${utils.formatarMoeda(orc.valorOrcamento)} | Custos: ${utils.formatarMoeda(resultadoFinanceiro.custos)}\n\n`;
        if(!confirm(mensagemPrejuizo)) {
    ...
    try {
        ...
        setTimeout(() => {
            if (typeof window.editarPedido === 'function') {
                ...
                if(utils && utils.showToast) {
                    utils.showToast(`Pedido ${resultado.numero} gerado. Revise os dados financeiros.`, 'info');
                }
            } else {
