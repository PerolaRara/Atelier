...
// [NOVO] Variável para armazenar o callback de atualização (Padrão Observer)
let onProductUpdateCallback = null;
...
// [NOVO] Função para registrar o observador de atualizações de produto
export function setOnProductUpdateCallback(callback) {
    onProductUpdateCallback = callback;
}
...
export async function atualizarCustosProdutosPorMaterial(material) {
    ...
    for (const prod of produtosAfetados) {
        ...
        // [NOVO] Notifica o sistema (Precificação) que o custo deste produto mudou
        if (onProductUpdateCallback) {
            await onProductUpdateCallback(prod);
        }
    }
